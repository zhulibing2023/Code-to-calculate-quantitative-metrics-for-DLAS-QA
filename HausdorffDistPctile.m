function [hd, meanD, D, minP, minQ] = HausdorffDistPctile(P,Q,pct,lmf,dv)
...
vq=prctile(minQ, pct);
meanq=mean(sqrt(minQ));
hd = (sqrt(vp)+sqrt(vq))/2;
hd(end)=sqrt(max([vp(end),vq(end)]));
meanD=(meanp+meanq)/2;
D = [];
minQ=sqrt(minQ); 
minP=sqrt(minP);    
